...
class VentanaSugerencias(tk.Toplevel):
    def __init__(self, parent, sugerencias, texto_original, controller):
        ...
        ttk.Button(marco_botones, text="Leer en voz alta", command=self.leer_sugerencia).pack(side="left", padx=5)
    ...
    def leer_sugerencia(self):
        ...
        if seleccion:
            ...
            self.controller.datos_compartidos["registro_retroalimentacion"].append({
                "frase": frase,
                "tipo": "leída en voz alta",
                "contexto": self.texto_original
            })
        else:
            messagebox.showwarning("Advertencia", "Selecciona una frase para leerla.")
