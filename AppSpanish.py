        ...
        try:
            ...
            if archivo:
                with open(archivo, 'w', newline='', encoding='utf-8') as csvfile:
                    ...
                    writer.writerow(["Fecha", "Categoría", "Frase Sugerida", "Tipo de Evaluación", "Frase Original"])
                    for registro in self.datos_compartidos["registro_retroalimentacion"]:
                        writer.writerow([
                            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            self.datos_compartidos["categoria"].get(),
                            registro["frase"],
                            registro["tipo"],
                            registro["contexto"]
                        ])
